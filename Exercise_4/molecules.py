...
def calc_dist(mol1, mol2, Cn, Ce):
    ...
    upper_left = [a != b for a, b in itertools.product(mol1.atoms, mol2.atoms)]  # check where the symbols are not equal
...
# get the validation and the training set (this is very fast)
...
print()
print('Cn = {} and Ce = {}'.format(Cn, Ce))

for k in K:

    # for each validation-molecule, get the k indices of test-molecules with the smallest distance
    ...
    acc = np.sum(np.equal(truth, pred))/len(valid)
